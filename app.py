...
def process_data(data, max_techs, priority_weight, distance_weight):
    ...
    if 'status' in valid_data.columns:
        try:
            ...
            st.write(f"🔍 **Status values found:** {valid_data['status'].unique()}")
            
            before_filter = len(valid_data)
            valid_data = valid_data[
                valid_data['status'].str.lower().isin(['open', 'pending', 'unassigned'])
            ]
            st.write(f"✅ **Rows with status 'open/pending/unassigned':** {len(valid_data)} (filtered out {before_filter - len(valid_data)})")
...
with st.sidebar:
    ...
    **Nice to have:**
    ...
    # Quick help buttons
    help_topics = {
        "How do I upload data?": "📤 Click 'Browse files' above to upload your CSV file. You can also drag and drop it directly into the upload box!",
        "What columns do I need?": "📊 Your CSV must have columns containing 'latitude' and 'longitude' (can be named Customer_Latitude, lat, Location_Lat, etc.). Other helpful columns: dispatch_id, status, priority, skills, appointment times.",
        "Why is my data filtered?": "🔍 The optimizer only uses dispatches with status 'open', 'pending', or 'unassigned'. Check the debug output to see what status values are in your data.",
        "How does optimization work?": "🧠 The tool clusters jobs geographically, assigns them to technicians, then optimizes each tech's route using distance and priority. It balances workload across all techs.",
        "Can I change settings?": "⚙️ Yes! Use the sliders above to adjust the number of technicians, priority weight (favor urgent jobs), and distance weight (favor shorter routes).",
        "How do I download results?": "💾 After optimization, scroll down to see a 'Download Results' button at the bottom. Click it to get a CSV with optimized routes and technician assignments."
    }
    
    # Display quick help buttons
    selected_topic = st.selectbox(
        "Quick Help Topics:",
        ["Select a question..."] + list(help_topics.keys()),
        key="help_topic"
    )
    
    if selected_topic != "Select a question...":
        st.info(f"💡 **{selected_topic}**\n\n{help_topics[selected_topic]}")
    
    # Chat input for custom questions
    with st.expander("🤖 Ask a Custom Question"):
        user_question = st.text_input("Type your question here:", key="custom_question")
        
        if user_question:
            # Simple keyword-based responses
            response = "🤔 I'm a simple helper bot! Try selecting from the quick help topics above for common questions. For specific issues, check the debug output when you run an optimization."
            
            # Keyword matching
            question_lower = user_question.lower()
            if any(word in question_lower for word in ['upload', 'file', 'csv', 'data']):
                response = help_topics["How do I upload data?"]
            elif any(word in question_lower for word in ['column', 'field', 'latitude', 'longitude', 'require']):
                response = help_topics["What columns do I need?"]
            elif any(word in question_lower for word in ['filter', 'status', 'missing', 'empty']):
                response = help_topics["Why is my data filtered?"]
            elif any(word in question_lower for word in ['work', 'optimize', 'algorithm', 'how']):
                response = help_topics["How does optimization work?"]
            elif any(word in question_lower for word in ['setting', 'change', 'adjust', 'slider']):
                response = help_topics["Can I change settings?"]
            elif any(word in question_lower for word in ['download', 'save', 'export', 'result']):
                response = help_topics["How do I download results?"]
