...
def process_data(data, max_techs, priority_weight, distance_weight):
    ...
    # Auto-detect columns with more flexible matching
    column_mapping = {}
    
    # Find latitude column - now includes exact match for Customer_latitude
    for col in data.columns:
        if col not in column_mapping.values():
            if any(term in col for term in ['lat', 'latitude', 'y_coord', 'y', 'customer_latitude']):
                if 'long' not in col and 'lng' not in col:
                    column_mapping['latitude'] = col
                    break
    
    # Find longitude column - now includes exact match for Customer_longitude
    for col in data.columns:
        if col not in column_mapping.values():
            if any(term in col for term in ['lon', 'long', 'lng', 'longitude', 'x_coord', 'x', 'customer_longitude']):
    ...
    if 'latitude' not in column_mapping or 'longitude' not in column_mapping:
        raise ValueError(
            ...
            f"Your columns are: {', '.join(original_columns)}\n\n"
            f"Please make sure your CSV has columns for latitude and longitude.\n"
            f"Accepted column names:\n"
            f"  • Latitude: 'lat', 'latitude', 'Lat', 'Latitude', 'y', 'Y'\n"
            f"  • Longitude: 'lon', 'long', 'lng', 'longitude', 'Lon', 'Long', 'x', 'X'"
        )
