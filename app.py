...
def process_data(data, max_techs, priority_weight, distance_weight):
    ...
    if 'latitude' in column_mapping:
        st.info(f"✅ Found latitude column: `{column_mapping['latitude']}`")
    if 'longitude' in column_mapping:
        st.info(f"✅ Found longitude column: `{column_mapping['longitude']}`")
